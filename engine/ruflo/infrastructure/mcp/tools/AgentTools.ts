/**
 * AgentTools — MCP tools for agent management operations.
 */
...
export class AgentTools implements MCPToolProvider {
  ...
  private async spawnAgent(config: AgentConfig): Promise<MCPToolResult> {
    ...
    if (!validTypes.includes(config.type)) {
      return {
        success: false,
        error: `validation: type must be one of ${validTypes.join(', ')}`,
      };
    }
    ...
    if (config.capabilities && !Array.isArray(config.capabilities)) {
      return {
        success: false,
        error: 'validation: capabilities must be an array',
      };
    }
